...
exports.helpCommand = {
    data: new discord_js_1.SlashCommandBuilder().setName("도움말").setDescription("VALO LOUNGE 봇에서 사용할 수 있는 명령어 목록을 확인합니다."),
    async execute(interaction) {
        const member = interaction.guild
            ? await interaction.guild.members.fetch(interaction.user.id).catch(() => null)
            : null;
        const isAdmin = (0, permissions_1.hasStatsRole)(member);
