...
async function handleMarket(interaction) {
    ...
    const stocks = await prisma_1.prisma.stock.findMany({ orderBy: { symbol: "asc" } });
    const embed = new discord_js_1.EmbedBuilder()
        .setColor(embed_1.CASINO_TEAL)
        .setAuthor({ name: "💹 VALO LOUNGE STOCK EXCHANGE" })
        .setTitle("「 📈 실시간 시세 」")
        .addFields(stocks.map((s) => {
...
async function handleBuy(interaction) {
    ...
    const stock = await prisma_1.prisma.stock.findUnique({ where: { symbol } });
    if (!stock) {
    ...
    const existing = await prisma_1.prisma.stockHolding.findUnique({
    ...
    const [, casinoPoint] = await prisma_1.prisma.$transaction([
        prisma_1.prisma.stockHolding.upsert({
            where: { userId_stockId: { userId: user.id, stockId: stock.id } },
            update: { quantity: newQty, avgBuyPrice: newAvg },
            create: { userId: user.id, stockId: stock.id, quantity, avgBuyPrice: stock.price },
        }),
        prisma_1.prisma.casinoPoint.update({ where: { userId: user.id }, data: { points: { decrement: cost } } }),
...
async function handleSell(interaction) {
    ...
    const holding = await prisma_1.prisma.stockHolding.findUnique({
        where: { userId_stockId: { userId: user.id, stockId: stock.id } },
    });
    ...
    const [, casinoPoint] = await prisma_1.prisma.$transaction([
        remainingQty > 0
            ? prisma_1.prisma.stockHolding.update({ where: { id: holding.id }, data: { quantity: remainingQty } })
            : prisma_1.prisma.stockHolding.delete({ where: { id: holding.id } }),
    ...
    if (casinoPoint.stockRealizedPl >= 10000) {
        const unlocked = await (0, achievement_service_1.tryUnlockAchievement)(user.id, "STOCK_PROFIT_10000");
        if (unlocked)
...
async function handlePortfolio(interaction) {
    await interaction.deferReply();
    const user = await (0, points_service_1.ensureUser)(interaction.user.id, interaction.user.username);
    const holdings = await prisma_1.prisma.stockHolding.findMany({
...
exports.stockCommand = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addStringOption((opt) => opt.setName("종목").setDescription("매수할 종목").setRequired(true).addChoices(...STOCK_CHOICES))
        .addIntegerOption((opt) => opt.setName("수량").setDescription("매수 수량").setRequired(true).setMinValue(1)))
        .addSubcommand((sub) => sub
        .setName("매도")
        .setDescription("보유 종목을 CP로 매도합니다.")
        .addStringOption((opt) => opt.setName("종목").setDescription("매도할 종목").setRequired(true).addChoices(...STOCK_CHOICES))
    ...
    async execute(interaction) {
        ...
        if (sub === "시장")
            return handleMarket(interaction);
        if (sub === "매수")
