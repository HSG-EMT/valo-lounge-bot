export interface RodTier {
  name: string;
  emoji: string;
  cost: number; // CP cost to upgrade into this tier (0 for the free starter rod)
  rareBonus: number; // shifts fish odds toward rarer grades — see rollFish()
}
...
export const FISH_GRADES: FishGrade[] = [
...
export function rollFish(rodTier: number): { grade: FishGrade; amount: number } {
  const bonus = ROD_TIERS[rodTier]?.rareBonus ?? 0;
  const weights = FISH_GRADES.map((g, i) => (i <= 1 ? g.weight * (1 - bonus) : g.weight * (1 + bonus * i)));
  const total = weights.reduce((a, b) => a + b, 0);

  let roll = Math.random() * total;
  let grade = FISH_GRADES[FISH_GRADES.length - 1];
  for (let i = 0; i < FISH_GRADES.length; i++) {
    if (roll < weights[i]) {
      grade = FISH_GRADES[i];
      break;
    }
