...
| `/레벨 [대상:<유저>]` | 레벨/등급/XP 진행바, 누적 등급 혜택 확인 | 있음 (`UserLevel`) |
| `/등급보너스` | 플래티넘 등급(Lv.51) 이상, 하루 한 번 CP 보너스 수령 | 있음 (`CasinoPoint`, `StatusLog`) |
...
메시지·음성채널 활동, 출석체크, 미니게임 플레이로 경험치(XP)를 얻어 최대 100레벨까지 성장하는 시스템입니다. `UserLevel.totalXp`(누적 XP)만 저장하고, 레벨/등급은 그때그때 `src/config/levels.ts`의 곡선으로 계산합니다 — 곡선 공식을 나중에 바꿔도 기존 유저 데이터가 깨지지 않습니다.

**XP 획득량** (`src/config/levels.ts` 상수로 조정 가능)

| 활동 | XP |
|---|---|
| 메시지 1개 | 2 |
| 음성채널 1분 | 1 |
| 출석체크 | 30 (실버 등급 이상 +20 보너스) |
| 미니게임 1회 (`/주사위`·`/코인`·`/카지노`·`/행운`·`/낚시`·`/낚시대상점` 구매·`/슬롯`·`/주식` 매수·매도·`/팀짜기`) | 5 |

**레벨업 난이도**: 레벨 N → N+1에 필요한 XP는 `100 + N×20`으로 갈수록 늘어납니다 (레벨 1→2는 120XP, 레벨 99→100은 2,080XP). 레벨업/등급승급 시 해당 명령어의 결과 임베드에 배너가 자동으로 붙습니다(업적 달성 배너와 같은 방식).

**등급 (브론즈~마스터, 6단계)** — 상위 등급 혜택은 하위 등급 혜택을 포함해 누적됩니다.

| 등급 | 레벨 구간 | 새로 해금되는 혜택 |
|---|---|---|
| 🥉 브론즈 | 1~10 | 일반 라운지 멤버 |
| 🥈 실버 | 11~30 | 출석체크 경험치 추가 획득 |
| 🏅 골드 | 31~50 | 등급 뱃지 표시 |
| 💠 플래티넘 | 51~75 | 하루 1회 CP 보너스 (`/등급보너스`) |
| 💎 다이아몬드 | 76~95 | 고급 랜덤상자 선물(준비 중), 전용 칭호 |
| 👑 마스터 | 96~100 | 희귀 랜덤상자 선물(준비 중), 최상위 VIP 칭호 |

- 골드 등급의 "디스코드 전용 역할"과 다이아몬드/마스터의 "칭호"는 실제 Discord 역할 지급이나 닉네임 변경 없이, `/레벨`·`/통계` 같은 명령어 결과에 **텍스트로만** 표시됩니다.
- 랜덤상자 선물은 아직 자동 지급 로직이 없습니다 — 다이아몬드/마스터 등급이면 혜택 목록에 뜨기만 하고, 실제 지급은 관리자가 수동으로 처리해야 합니다 (다음 단계 후보).
- `/등급보너스`는 플래티넘 미만이면 안내만 뜨고, 플래티넘 이상이면 `/코인`과 동일한 24시간 쿨타임으로 CP 100을 지급합니다.
...
- 랜덤 에이전트
- User Stat, Riot Login 연동, 시즌 시스템, 이벤트 자동화
- 랜덤박스: 웹(`valo-lounge-server`)의 `POST /events/boxes/:id/draw`는 실제 추첨 로직이 구현되어 있지만, 봇에서 같은 기능을 슬래시 명령어로 트리거하는 것은 아직 없음
- 다이아몬드/마스터 등급의 랜덤상자 자동 선물 (현재는 혜택 목록에 표시만 되고 수동 지급)
- 골드 등급 이상 Discord 역할 자동 지급, 다이아몬드/마스터 칭호의 실제 닉네임 반영 (현재는 봇 임베드 텍스트로만 표시)
