...
- `STATS_ROLE_IDS` (선택) — `/통계` 명령어를 쓸 수 있는 역할 ID들 (콤마 구분). 아래 "사용량 통계 조회" 참고. 비워두면 아무도 `/통계`를 못 씁니다.
...
| `/통계 유저 대상:<유저>` | 특정 유저의 누적 음성 채널 시간·메시지 수 조회 (**관리자 전용**) | 있음 (`VoiceSession`, `MessageStat`) |
| `/통계 보이스순위 [인원수]` | 전체 유저 음성 채널 사용량 순위 (**관리자 전용**) | 있음 (`VoiceSession`) |
| `/통계 메시지순위 [인원수]` | 전체 유저 메시지 수 순위 (**관리자 전용**) | 있음 (`MessageStat`) |
| `/통계 전체` | 서버 전체 통계 요약 (누적 메시지·음성시간, 활성 유저 수 등) (**관리자 전용**) | 있음 |
...
`STATS_ROLE_IDS`(콤마 구분 역할 ID 목록)에 있는 역할 중 하나라도 가진 사람만 실행할 수 있습니다 (권한 없으면 "이 명령어를 사용할 권한이 없습니다" ephemeral 응답). 4개 서브커맨드:

- `유저 대상:<유저>` — 특정 유저의 누적 음성 채널 시간 + 누적/오늘 메시지 수
- `보이스순위 [인원수]` — 전체 유저 음성 사용량 순위 (기본 10명, 최대 25명)
- `메시지순위 [인원수]` — 전체 유저 메시지 수 순위
- `전체` — 서버 전체 요약 (현재 인원, 현재 음성 접속 인원, 누적 메시지/음성시간, 각각 활동한 유저 수)

메시지 수는 `src/events/messageStats.ts`(`MessageCreate` 리스너)가 봇 메시지를 제외한 모든 메시지마다 `MessageStat.totalCount`를 1씩 증가시키며 별도로 집계합니다 (메시지 원문은 저장하지 않고 개수만 셉니다 — 위 메시지 수정/삭제 로그와는 별개 데이터). 음성 사용량은 이미 기록 중인 `VoiceSession`을 그대로 재사용하며, 아직 접속 중인(퇴장 안 한) 세션은 조회 시점까지의 경과 시간을 더해서 "지금 기준" 총량을 보여줍니다.
